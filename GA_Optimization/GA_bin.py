...
# from Helper import Intensity, stddev
import argparse
...
parser.add_argument("--mut_prob", type=float, default=0.05, help="Mutation Probability")
...
Function for initialising a generation (each coordinate is represented as a 7-bit binary string)"""
def Generate_initial_gen_bin():
  new_gen=[[0 for i in range(49)] for j in range(14)]
  for i in range(0,14):
    for j in range(0,49):
      new_gen[i][j]= str(random.randint(0,1))
  return new_gen

xw=np.linspace(0,1,opt.num_grid_points)        #Dividing points on the wall in a 10*10 grid with z coordinate of wall=1
yw=np.linspace(0,1,opt.num_grid_points)
zw=1

"""Function for getting the new set of 'better' parents from the current generation"""
def Parent_pool_bin(new_gen):
  avg_J=0
  ...
  for i in range(0,14):
    ...
    for j in range(0,opt.num_grid_points):
      for k in range(0,opt.num_grid_points):
        intensity0[j][k]=Intensity(int("".join(new_gen[i][0:7]),2)/127,int("".join(new_gen[i][7:14]),2)/127,int("".join(new_gen[i][14:21]),2)/127,int("".join(new_gen[i][21:28]),2)/127,int("".join(new_gen[i][28:35]),2)/127,int("".join(new_gen[i][35:42]),2)/127,int("".join(new_gen[i][42:49]),2)/127,xw[j],yw[k])
  ...
  t=0;
  # print (min_intensity)
  Parents = [[0 for j in range(49)] for k in range(7)] ####
...
def Generate_new_gen_bin(Parents):
  new_gen=[[0 for i in range(49)] for j in range(14)];
  b=1
  for i in range(0,7):
    index=0;
    while (b):
      ...
      b=bool(index==i);
    child1, child2=mate_bin(Parents[i],Parents[index])
    for k in range(0,49):
...
new_gen=Generate_initial_gen_bin();
current_iter=0
while(current_iter<opt.num_iter):
  parents=Parent_pool_bin(new_gen);
  new_gen=Generate_new_gen_bin(parents);
  current_iter=current_iter+1;
ans= best_bin(parents)
print ("Bulb 1 at x = %f,y = %f,z = %f" % (int("".join(parents[ans][0:7]),2)/127,int("".join(parents[ans][7:14]),2)/127,int("".join(parents[ans][42:49]),2)/127))
print ("Bulb 2 at x = %f,y = %f,z = %f" % (int("".join(parents[ans][14:21]),2)/127,int("".join(parents[ans][21:28]),2)/127,int("".join(parents[ans][42:49]),2)/127))
print ("Bulb 3 at x = %f,y = %f,z = %f" % (int("".join(parents[ans][28:35]),2)/127,int("".join(parents[ans][35:42]),2)/127,int("".join(parents[ans][42:49]),2)/127))
